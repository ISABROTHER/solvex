...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (_event, session) => {
        ...
        // Reset sensitive state on auth change
        setProfile(null);
        setAssignments([]);
        setDocuments([]);
        ...
        if (currentUser) {
          try {
            ...
            // This is the top 1% fix: one concurrent fetch for all remaining data
  ...
  useEffect(() => {
    ...
    // We use refetchAll for anything that changes the main list structure (add/delete document, add/remove assignment)
    const refetchAll = () => fetchEmployeeData(user); 
    
    // We use refetchAssignment for minor changes like new comments on an open assignment
    const refetchAssignment = async (assignmentId: string) => {
      try {
        const { data, error } = await getFullAssignmentDetails(assignmentId);
        if (error) throw error;
        // This is a global state update to keep the open dashboard panel fast
