...
interface AuthState {
  ...
  isLoading: boolean; // Tracks initial load and login attempts
}
...
const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
     ...
     if (session?.user) {
        ...
        try {
            ...
            if (profileError) {
                ...
                // Profile found, but role is missing or invalid
  ...
  const login = async (email: string, password: string): Promise<LoginResult> => {
    ...
    try {
      ...
      if (role === null && data.session) { // Check data.session to ensure login itself was okay
          console.warn("Login successful, but failed to determine user role from profile.");
          // Error message should already be set by updateUserState
          setAuthState(prev => ({ ...prev, isLoading: false })); // Stop loading
          // Log user out if role is strictly required? Or let MyPage handle the error display.
          // await logout(); // Optional: force logout if role is essential
      ...
      // Catch errors from signInWithPassword or updateUserState
  ...
  const logout = async () => {
    ...
    try {
        ...
        // State clears via the onAuthStateChange listener
        navigate('/my-page'); // Redirect after successful sign out
    } catch (err: any) {
        console.error("Logout failed:", err.message);
        setError(err.message || 'Logout failed.');
        setAuthState(prev => ({ ...prev, isLoading: false })); // Clear loading on error
    }
    // isLoading will be set to false by the onAuthStateChange listener eventually
  };
