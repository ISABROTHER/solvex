...
### **B. Update Supabase Operations and Forms Submission Logic**

We update the `operations.ts` to fetch from the correct table and update `forms.ts` to insert into it.

**File:** `src/lib/supabase/operations.ts` (Changed two functions: `getCareerApplications` and `updateCareerApplicationStatus`)

```typescript:Supabase Operations:src/lib/supabase/operations.ts
[Immersive content redacted for brevity.]
```eof

**File:** `src/lib/supabase/forms.ts` (Changed submission target)

```typescript:Supabase Forms:src/lib/supabase/forms.ts
[Immersive content redacted for brevity.]
```eof

---

## **Step 3: New Dashboard Tab (`SubmittedApplicationsTab.tsx`)**

We create a new, dedicated component for the dashboard, and then update the main navigation.

### **A. New Dashboard Component (`src/pages/admin/DashboardPage/tabs/SubmittedApplicationsTab.tsx`)**

This is the fully functional, real-time replacement for the old Applications Tab.

```tsx:New Submitted Applications Tab:src/pages/admin/DashboardPage/tabs/SubmittedApplicationsTab.tsx
// src/pages/admin/DashboardPage/tabs/SubmittedApplicationsTab.tsx
// @ts-nocheck
...
import { getSubmittedApplications, updateSubmittedApplicationStatus } from "../../../../lib/supabase/operations"; // Use new functions
import { supabase } from "../../../../lib/supabase/client";
import type { Database } from "../../../../lib/supabase/database.types";
import { useToast } from "../../../../contexts/ToastContext";
import { motion, AnimatePresence } from "framer-motion";

// Define the joined type based on the new schema and aliasing in operations.ts
type ApplicationRow = Database['public']['Tables']['submitted_applications']['Row'] & {
    job_position: { title: string, team_name: string | null } | null;
};
...
// --- MODAL COMPONENT (Reused/Refactored) ---
const ApplicationDetailModal: React.FC<{ application: Application, isOpen: boolean, onClose: () => void, onUpdateStatus: (id: string, newStatus: string) => void }> = ({ application, isOpen, onClose, onUpdateStatus }) => {
    ...
    // FIX: Use job_position for linked data, falling back to position_title
...
const SubmittedApplicationsTab: React.FC = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        const { data, error: fetchError } = await getSubmittedApplications();

        if (fetchError) {
            setError("Failed to fetch applications. Check RLS configuration or browser console.");
    ...
    useEffect(() => {
        ...
        const channel = supabase.channel('public:submitted_applications')
          .on('postgres_changes', { event: '*', schema: 'public', table: 'submitted_applications' }, () => fetchData())
          .subscribe();
    ...
    const handleUpdateStatus = async (id: string, newStatus: string) => {
        try {
            const { error: updateError } = await updateSubmittedApplicationStatus(id, newStatus);
    ...
    const filteredApplications = useMemo(() => {
        ...
        return applications.filter(app => 
            ...
            app.position_title?.toLowerCase().includes(lowerCaseSearch) ||
            app.job_position?.title?.toLowerCase().includes(lowerCaseSearch)
        );
    ...
    return (
        <div className="space-y-6">
            <Card title={`Submitted Applications (${applicationCount})`} right={
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {filteredApplications.length > 0 ? (
                                filteredApplications.map((app) => {
                                    ...
                                    const positionTitle = app.job_position?.title || app.position_title || 'Unknown Position';
                                ...
                                <tr>
                                    <td colSpan={6} className="text-center py-8 text-gray-500">
                                        {applications.length === 0 ? "No submitted applications found." : "No applications match your search term."}
                                    </td>
                                </tr>
                            )}
                        </tbody>
...
export default SubmittedApplicationsTab;